...
const RFCDropdownButton = ({ onSelect }) => {
  ...
  // Opciones fltradas dinámicamente
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {/* Dropdown */}
      {open && (
        <ul className="absolute right-0 mt-2 w-52 bg-[#143559]/55 text-white text-sm max-h-[550px] overflow-y-auto
                       rounded-md shadow-lg z-50 backdrop-blur-sm">
          {opcionesRFC.length > 10 && (
            <li className="p-2 sticky top-0 z-10">
              <input
                type="text"
                value={searchTerm}
          ...
          {filteredRFCs.map((opt, i) => (
            <li
              key={i}
              onClick={() => {
                onSelect(opt)
                setOpen(false)
                localStorage.setItem('rfc', opt)
                redirectOrRefresh()
              }}
